...
func TestSingerTls(t *testing.T) {
	cert, err := signHost(CertificateAuthority, []string{"example.com", "1.1.1.1", "localhost"})
	...
	defer server.Close()
	server.TLS = &tls.Config{Certificates: []tls.Certificate{*cert, CertificateAuthority}}
	server.TLS.BuildNameToCertificate()
	server.StartTLS()
	certpool := x509.NewCertPool()
	certpool.AddCert(CertificateAuthority.Leaf)
	tr := &http.Transport{
...
func TestSingerX509(t *testing.T) {
	...
	orFatal("CheckSignatureFrom", cert.Leaf.CheckSignatureFrom(CertificateAuthority.Leaf), t)
